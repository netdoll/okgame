...
namespace Gwen
{
	namespace Renderer
	{
		GwenRendererPS4::GwenRendererPS4( )//IDirect3DDevice9* pDevice 
		{
			//m_pD3D = NULL;
			//m_pDevice = pDevice;
			m_iVertNum = 0;
		...
		GwenRendererPS4::~GwenRendererPS4()
		{
		}

		void GwenRendererPS4::Begin()
		{
...
//			m_pDevice->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_DISABLE );
		...
		void GwenRendererPS4::AddVert( int x, int y )
		{
		...
		void GwenRendererPS4::AddVert( int x, int y, float u, float v )
		{
			if ( m_iVertNum >= MaxVerts - 1 )
			{
				Flush();
		...
		void GwenRendererPS4::DrawFilledRect( Gwen::Rect rect )
		{
		...
		void GwenRendererPS4::SetDrawColor( Gwen::Color color )
		{
			//m_Color = D3DCOLOR_ARGB( color.a, color.r, color.g, color.b );
		}

		void GwenRendererPS4::LoadFont( Gwen::Font* font )
		{
...
//			font->data = pFontData;
		...
		void GwenRendererPS4::FreeFont( Gwen::Font* pFont )
		{
...
//			}
		...
		void GwenRendererPS4::RenderText( Gwen::Font* pFont, Gwen::Point pos, const Gwen::UnicodeString & text )
		{
			Flush();
		...
		Gwen::Point GwenRendererPS4::MeasureText( Gwen::Font* pFont, const Gwen::UnicodeString & text )
		{
			// If the font doesn't exist, or the font size should be changed
			if ( !pFont->data || fabs( pFont->realsize - pFont->size * Scale() ) > 2 )
			{
...
//
		...
		void GwenRendererPS4::StartClip()
		{
//			Flush();
...
//			m_pDevice->SetScissorRect( &r );
		...
		void GwenRendererPS4::EndClip()
		{
//			Flush();
//			m_pDevice->SetRenderState( D3DRS_SCISSORTESTENABLE, FALSE );
		}

		void GwenRendererPS4::DrawTexturedRect( Gwen::Texture* pTexture, Gwen::Rect rect, float u1, float v1, float u2, float v2 )
		{
...
//			}
		...
		void GwenRendererPS4::LoadTexture( Gwen::Texture* pTexture )
		{
...
//			pTexture->height = ImageInfo.Height;
		...
		void GwenRendererPS4::FreeTexture( Gwen::Texture* pTexture )
		{
//			IDirect3DTexture9* pImage = ( IDirect3DTexture9* ) pTexture->data;
//
//			if ( !pImage ) { return; }
...
//			pImage->Release();
		...
		Gwen::Color GwenRendererPS4::PixelColour( Gwen::Texture* pTexture, unsigned int x, unsigned int y, const Gwen::Color & col_default )
		{
//			IDirect3DTexture9* pImage = ( IDirect3DTexture9* ) pTexture->data;
//
...
//			D3DXCOLOR color = pixels[lockedRect.Pitch / sizeof(uint32) * y + x];
//			pSurface->UnlockRect();
//			pSurface->Release();
			return Gwen::Color();// color.r * 255, color.g * 255, color.b * 255, color.a * 255 );
		}

		void GwenRendererPS4::Release()
		{
		...
		void GwenRendererPS4::FillPresentParameters( Gwen::WindowProvider* pWindow )//, D3DPRESENT_PARAMETERS & Params
		{
...
//			Params.PresentationInterval			= D3DPRESENT_INTERVAL_IMMEDIATE;
		...
		bool GwenRendererPS4::InitializeContext( Gwen::WindowProvider* pWindow )
		{
//			HWND pHWND = ( HWND ) pWindow->GetWindow();
...
//			uint32 BehaviourFlags = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
//
...
//			}
		...
		bool GwenRendererPS4::ShutdownContext( Gwen::WindowProvider* pWindow )
		{
//			if ( m_pDevice )
//			{
//				m_pDevice->Release();
...
//			}
		...
		bool GwenRendererPS4::PresentContext( Gwen::WindowProvider* pWindow )
		{
			//m_pDevice->Present( NULL, NULL, NULL, NULL );
			return true;
		}

		bool GwenRendererPS4::ResizedContext( Gwen::WindowProvider* pWindow, int w, int h )
		{
		...
		bool GwenRendererPS4::BeginContext( Gwen::WindowProvider* pWindow )
		{
		...
		bool GwenRendererPS4::EndContext( Gwen::WindowProvider* pWindow )
		{
			//m_pDevice->EndScene();
			return true;
		}
