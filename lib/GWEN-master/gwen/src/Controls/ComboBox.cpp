...
GWEN_CONTROL_CONSTRUCTOR( ComboBox )
{
	...
	//m_Base = new Controls::Base(this);
	//m_Base->Dock(Pos::Top);
	//m_Base->SetSize(200,200);
...
{
	Base::List & children = m_Menu->m_Base->GetChildren();
...
void ComboBox::OpenList()
{
	...
	//m_Base->SetParent(GetCanvas());
	//m_Base->SetHidden(false);
	//m_Base->BringToFront();
	...
	//m_Menu->SetAutoHideBars(false);
	//m_Menu->SetScroll(true, true);
	
	Gwen::Point p = LocalPosToCanvas(Gwen::Point(0, 0));
	

//	int h = m_Menu->Height();
//	std::cout << h << std::endl;//422
//	std::cout << GetCanvas()->GetBounds().h << std::endl;//800
//	std::cout << p.y << std::endl;//358
//	std::cout << Height() << std::endl;//20
//
//	if(p.y + h>GetCanvas()->GetBounds().h)
//	{
//		h = GetCanvas()->GetBounds().h - p.y;
//	}

	m_Menu->SetBounds( Gwen::Rect( p.x, p.y + Height(), Width(), m_Menu->Height()) );
	//m_Menu->Dock(Pos::Fill);
	//m_Menu->SetInnerSize(Width(), 1000);
	//m_Menu->UpdateScrollBars();
	//m_Menu->Invalidate();
	
	//m_Menu->SetHeight(200);
