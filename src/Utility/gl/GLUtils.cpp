...
if (dm == nullptr) 
{
	log.error("SDL_GetCurrentDisplayMode failed: "+string(SDL_GetError()));
} else {
	monitorWidth = dm->w;
	monitorHeight = dm->h;
	log.info("Desktop display mode is "+to_string(dm->w)+"x" + to_string(dm->h) + "px.");
}
...
window = SDL_CreateWindow(windowName, w, h, SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL);
	...
	if (displays) {
		for (int i = 0; i < num_displays; i++)
		{
			...
			else
			{
				...
				SDL_Log("Display #%d: current display mode is %dx%dpx.", i, mode->w, mode->h);
...
{//=========================================================================================================================
	...
	if (displays)
	{
		...
		{
			int numDisplayModes;
			SDL_DisplayMode **modes = SDL_GetFullscreenDisplayModes(displays[d], &numDisplayModes);
			if (modes) {
...
{//===========================================================================================================================
	...
	//if (surface->format->BytesPerPixel < 4)
	{
		...
		temp = SDL_CreateSurface(texWidth, texHeight, SDL_PIXELFORMAT_RGBA32);
