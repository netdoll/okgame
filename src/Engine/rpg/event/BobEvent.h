...
class BobEvent : public ServerObject
{
public:
	static Logger log;
...
public:
	//BobEvent(Engine* g, int id);

	//for cutscenes only, string is not used, only to make unambiguous
	BobEvent(Engine* g, EventData* eventData, string s);

	BobEvent(Engine* g, EventData* eventData, Map* m);
	BobEvent(Engine* g, EventData* eventData, Area* a);
	BobEvent(Engine* g, EventData* eventData, Entity* e);
	BobEvent(Engine* g, EventData* eventData, Sprite* s);
	BobEvent(Engine* g, EventData* eventData, Door* d);
	void initEvent();
...
private:
	...
	void setShowAllBobStatusBarCaptions_BOOL();

	void setShowBobStatusBar_BOOL();
