...
{//=========================================================================================================================
  ...
  //oh it is also so that i can call init code from multiple constructors, oops
	...
	if(music!=nullptr)music->setVolume((((float)Main::globalSettings->musicVolume) / 100.0f));
...
{//=========================================================================================================================
	...
	log.debug("Loading graphics took " + to_string((double)((now - start) * 1000) / System::GetPerformanceFrequency()) + "ms");
	start = System::getPerformanceCounter();
	...
	now = System::getPerformanceCounter();
	log.debug("Loading sprites took " + to_string((double)((now - start) * 1000) / System::GetPerformanceFrequency()) + "ms");
	start = System::getPerformanceCounter();
	...
	now = System::getPerformanceCounter();
	log.debug("Loading GameTypes and GameSequences took " + to_string((double)((now - start) * 1000) / System::GetPerformanceFrequency()) + "ms");
	start = System::getPerformanceCounter();


	totalNow = System::getPerformanceCounter();
	log.debug("BobsGame::initAssets took " + to_string((double)((totalNow - totalStart) * 1000) / System::GetPerformanceFrequency()) + "ms");
...
{//=========================================================================================================================
	...
	GLUtils::fillBufferWithTransparency();
	...
	if (GLUtils::useShaders)
	{
		...
		//draw Blocks From Main FBO Attachment1 Into Bloom Attachment0 Using Bloom Shader
		...
		GLUtils::setupBloomThresholdShader();

		GLUtils::drawBobsGameFBOBloomMaskTexture();//draw the blocks in the mask texture to the bloom fbo attachment 0 using the bloom threshold shader
		...
		{
			GLUtils::doBlurShader();

		}

		GLUtils::setBobsGameMainFBO(); //draw to mainFBO texture 0, has the playing field

		GLUtils::enableBlend();
		

		GLUtils::bloomIntensity = g->currentGameType->bloomIntensity;
		...
		GLUtils::drawBlocksUsingBloomShader();
...
//		}
		...
		GLUtils::setBobsGameMaskFBO();
		GLUtils::fillBufferWithTransparency();
	
		GLUtils::drawBobsGameMainFBOTexture0();
		...
		GLUtils::drawBobsGameFBOBloomMaskTexture1();
...
//		}
		...
		//--------------------------
		GLUtils::setMainScreenFB();
		GLUtils::fillBufferWithBlack();


		GLUtils::drawPreColorFilterFBOTexture();
	...
	else
	{
		...
		if (GLUtils::useShaders)
		{
			...
			int shaderInt = 0;
			if(GLUtils::bgShaders.size()>=shaderCount)shaderInt = GLUtils::bgShaders.get(shaderCount)->value();
		...
		for (int i = 0; i < players.size(); i++)
		{
			...
			GLUtils::drawBobsGameFBO(g->playingFieldX0, g->playingFieldX1, g->playingFieldY0, g->playingFieldY1);
...
//			}
		...
		// draw MAIN FBO texture into SCREEN BUFFER
		//--------------------------
		GLUtils::setMainScreenFB(); //set the framebuffer back to the screen buffer
		GLUtils::fillBufferWithBlack();

		GLUtils::drawPostColorFilterFBOTexture();
	}
...
{//=========================================================================================================================
	...
	{
		if (music != nullptr && music->isPlaying() == false)
		{
			music->unpause();
		}
	}
...
{//=========================================================================================================================
	...
	BobFile builtInDataPathDir(builtInDataPathString);
	BobFile downloadedDataPathDir(downloadedDataPathString);
	...
	{
		vector<string> files;
		if (i == 0)files = builtInDataPathDir.list();
		if (i == 1)files = userDataPathDir.list();
		if (i == 2)files = downloadedDataPathDir.list();
		vector<string>::iterator it = files.begin();
		for (; it != files.end(); ++it)
		{
			//cout << *it << endl;
...
{//=========================================================================================================================
	...
	//Path filePath();
	BobFile file(userDataPathString + name);
...
{//=========================================================================================================================
	...
	vector<string> files = userDataPathDir.list();
...
{//=========================================================================================================================
	...
	string filePath = userDataPathString + filename;
	BobFile file(filePath);
	int version = 0;
	...
	if (file.exists())
	{
		while (file.exists())
		{
			file = BobFile(userDataPathString + gs->uuid + "." + to_string(version));
			version++;
		}
		file = BobFile(filePath);
...
{//=========================================================================================================================
	...
	if (file.exists())
	{
		...
		file = BobFile(userDataPathString + filename);
		file.renameTo(userDataPathString + gs->uuid + "." + to_string(version));
	}
