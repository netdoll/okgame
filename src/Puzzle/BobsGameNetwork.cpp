			...
			else
			{
				if (selectingHostedGame == false)
				{
					...
					if (roomsMenu->isSelectedID(FilterByMaxPlayers))
					{
						...
						if (!textStarted) { Main::StartTextInput(); getControlsManager()->text = num; textStarted = true; }
					...
					{
						chatEnabled = false;
						if (!textStarted) { Main::StartTextInput(); getControlsManager()->text = filterByKeyword; textStarted = true; }
					...
					{
						chatEnabled = false;
						if (!textStarted) { Main::StartTextInput(); getControlsManager()->text = friendUserName; textStarted = true; }
